...
def generate_ai_report(tickers):
    ...
    【任務一：既有清單深度診斷】
    請針對以下追蹤清單中的『每一隻股票』進行深度分析，嚴禁只複讀數據：
    {market_context}
    
    診斷要求：
    1. 必須主動分析基本面：結合該標的 2026 年的獲利預期、最新的重大新聞(如 VST 的收購、TSLA 的交付數據、NVDA 的晶片進度)。
    2. 針對清單中的所有股票，給出明確的基本面解釋，判斷是「利空出盡」還是「基本面轉壞」。
    3. 提及機構法人目前的目標價區間與評等建議。
    4. 必須對清單中的所有股票給予最詳盡的評論。
