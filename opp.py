...
def generate_ai_report(tickers):
    ...
    【任務一：清單診斷】
    請針對以下清單中的『每一隻股票』進行深度條列式分析：
    {market_context}
    
    排版格式規範 (嚴格遵守)：
    1. :orange[代號] (公司簡稱)
    * **現價**：:violet[數值]
    * **技術診斷**：分析 FKD 狀態（超買/超賣/背離）與量比籌碼意義。
    * **基本面分析**：主動檢索並說明該公司 2026 最新消息、獲利預期或產業地位 (特別是 VST 的核電收購案、TSLA 的交付動態、NVDA 的晶片進度)。
    * **建議標籤**：使用徽章標籤（例如 :green-badge[強烈建議買入]、:red-badge[建議減碼]、:violet-badge[低位攤平]、:gray-badge[觀望]）並簡述原因。
